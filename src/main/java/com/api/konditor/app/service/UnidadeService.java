 ...
 *
 * <p>Unidades são recursos globais (não pertencem a um workspace) e são usadas como referência nos
...
public class UnidadeService {
   ...
   *
   * @param tipo filtro opcional pelo tipo da unidade ({@code weight}, {@code volume}, {@code
   *     unit}). Quando {@code null}, retorna todas.
  ...
  public List<UnidadeResponse> listar(String tipo) {
    ...
    if (tipo != null && !tipo.isBlank()) {
      ...
      return unitRepository.findAllByTypeAndDeletedAtIsNullOrderByNameAsc(unitType).stream()
          .map(this::toResponse)
          .toList();
    }

    return unitRepository.findAllByDeletedAtIsNullOrderByNameAsc().stream()
        .map(this::toResponse)
        .toList();
  }

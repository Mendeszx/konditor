 ...
 * <p>Rotas:
 * <ul>
 *   <li>{@code POST /onboarding} — cria o workspace inicial do usuário recém-autenticado</li>
 * </ul>
 *
 * <p>Requer usuário autenticado via JWT. Os dados do usuário são extraídos
 * diretamente do token — nenhuma informação pessoal precisa ser enviada no body.
 */
@Slf4j
@RestController
...
public class OnboardingController {
     ...
     *
     * <p>Cria o workspace, vincula o usuário como {@code owner} e provisiona
     * a assinatura gratuita. Operação idempotente — retorna 422 se o usuário
     * já possui um workspace ativo.
     *
    ...
    public ResponseEntity<OnboardingResponse> realizarOnboarding(
        ...
        log.info("[ONBOARDING] Requisição recebida para usuário id={} email={}", usuario.id(), usuario.email());
        OnboardingResponse response = onboardingService.executarOnboarding(usuario, request);
        log.info("[ONBOARDING] Onboarding concluído. workspaceId={} userId={}", response.getWorkspaceId(), usuario.id());

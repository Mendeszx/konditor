 ...
 *
 * <ul>
 *   <li>{@code POST /receitas} — cria uma nova receita (rascunho por padrão)
 *   <li>{@code GET /receitas/{id}} — retorna detalhes completos de uma receita
 *   <li>{@code PUT /receitas/{id}} — atualiza uma receita (salvar rascunho)
 *   <li>{@code POST /receitas/{id}/publicar} — publica a receita (muda status para {@code
 *       publicada})
 *   <li>{@code POST /receitas/calcular} — calcula custos em tempo real, sem persistir
 *   <li>{@code GET /ingredientes} — autocomplete de ingredientes por nome
 * </ul>
 *
 * <p>Todos os endpoints requerem JWT válido. O tenant é resolvido automaticamente a partir do
 * {@code workspaceId} contido no token.
 */
@Slf4j
@RestController
...
public class ReceitaController {
   ...
   *
   * <p>Por padrão, a receita é criada como {@code rascunho}. Para criar já publicada, envie {@code
   * "status": "publicada"} no payload. O custo total e o preço sugerido são calculados
  ...
  public ResponseEntity<ReceitaResponse> criar(
    ...
    log.info(
        "[RECEITA] POST /receitas — userId={} workspaceId={}", usuario.id(), usuario.workspaceId());
  ...
  /**
   * Retorna os detalhes completos de uma receita pelo ID, incluindo lista de ingredientes com
   * custos calculados.
   *
  ...
  /**
   * Atualiza todos os campos de uma receita (salvar rascunho ou editar publicada). Os ingredientes
   * são completamente substituídos e os custos são recalculados.
   *
  ...
  /**
   * Publica uma receita, tornando-a visível no dashboard e listagens. Muda o status de {@code
   * rascunho} para {@code publicada}.
   *
  ...
  /**
   * Calcula custos e preço sugerido em tempo real para um conjunto de ingredientes, sem persistir
   * nenhum dado. Ideal para atualização dinâmica do painel lateral.
   *
   * <p>Os percentuais de mão de obra, custos fixos e margem desejada são opcionais (padrão: 20 %,
  ...
  public ResponseEntity<CustosCalculadosResponse> calcularCustos(
    ...
    log.info(
        "[RECEITA] POST /receitas/calcular — userId={} ingredientes={}",
        usuario.id(),
  ...
  /**
   * Retorna todas as categorias de receita globais, ordenadas por nome. Usadas para preencher chips
   * de filtro e o seletor de categoria ao criar/editar uma receita.
   *
  ...
  /**
   * Busca ingredientes do workspace por nome (autocomplete). Retorna todos os ingredientes ativos
   * quando {@code query} é omitido.
   *
  ...
  public ResponseEntity<List<BuscaIngredienteResponse>> buscarIngredientes(
    ...
    log.debug(
        "[INGREDIENTE] GET /ingredientes — query='{}' workspaceId={}",
        query,
        usuario.workspaceId());

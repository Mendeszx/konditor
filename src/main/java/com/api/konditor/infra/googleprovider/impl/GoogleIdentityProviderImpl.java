...
/**
 * Implementação do port {@link GoogleIdentityProvider} usando a biblioteca {@code
 * google-api-client}.
 *
 * <p>O {@link GoogleIdTokenVerifier} é construído uma única vez no startup e reutilizado em todas
 * as requisições — ele faz cache interno das chaves públicas do Google, evitando chamadas HTTP
 * desnecessárias.
 */
@Slf4j
@Component
public class GoogleIdentityProviderImpl implements GoogleIdentityProvider {
  ...
  public GoogleIdentityProviderImpl(@Value("${security.google.client-id}") String clientId) {
    this.verifier =

...
public interface ProductIngredientJpaRepository extends JpaRepository<ProductIngredientJpaEntity, UUID> {
    ...
    /**
     * Busca todos os vínculos produto-ingrediente que referenciam um determinado ingrediente,
     * com produto, ingrediente (com sua unidade base) e unidade da linha carregados.
     *
     * <p>Usado para recalcular os custos de todas as receitas afetadas quando
     * o custo de um ingrediente é atualizado.
     */
    @Query("""
            SELECT pi FROM ProductIngredientJpaEntity pi
            ...
            """)
    List<ProductIngredientJpaEntity> findAllByIngredientIdWithDetails(@Param("ingredientId") UUID ingredientId);
}

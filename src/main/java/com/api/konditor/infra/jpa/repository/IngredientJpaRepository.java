...
public interface IngredientJpaRepository extends JpaRepository<IngredientJpaEntity, UUID> {
  ...
  /**
   * Busca ingredientes do workspace cujo nome contém {@code query} (case-insensitive), com a
   * unidade carregada via JOIN FETCH para evitar N+1. Passando {@code query = ""} retorna todos os
   * ingredientes.
   */
  @Query(
      """

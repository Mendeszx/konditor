...
public class DashboardUseCaseImpl implements DashboardUseCase {
  ...
  public DashboardStatsResponse buscarEstatisticas(UsuarioAutenticado usuario) {
    ...
    if (receitas.isEmpty()) {
      log.info(
          "[DASHBOARD-STATS] Nenhuma receita ativa encontrada para workspaceId={}", workspaceId);
    ...
    log.info(
        "[DASHBOARD-STATS] Stats calculadas: total={} margemMedia={} melhorMargem={} ({}%)",
        receitas.size(), margemMedia, melhorProduto.getName(), melhorMargem.getMargem());
   ...
   *   <li>{@code calculatedCost} é nulo (sem ingredientes cadastrados — custo = 0 → margem = 100%)
   * </ul>
   */
  ...
  private ProductJpaEntity encontrarMelhorMargem(
      List<ProductJpaEntity> receitas, List<BigDecimal> margens) {
    int melhorIndice = 0;

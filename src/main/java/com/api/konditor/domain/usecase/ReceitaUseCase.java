...
public interface ReceitaUseCase {
  ...
  /**
   * Cria uma nova receita para o workspace autenticado. Calcula e persiste o custo total e o preço
   * sugerido com base nos ingredientes informados.
   */
  ReceitaResponse criar(CriarReceitaRequest request, UsuarioAutenticado usuario);

  /**
   * Atualiza todos os campos de uma receita existente. Recalcula custo e preço sugerido a partir
   * dos novos ingredientes.
   */
  ReceitaResponse atualizar(UUID id, CriarReceitaRequest request, UsuarioAutenticado usuario);

  /**
   * Publica uma receita, tornando-a visível no dashboard e demais listagens. Muda o status de
   * {@code rascunho} para {@code publicada}.
   */
   ...
   * nada. Ideal para atualização dinâmica do painel lateral no front-end.
   *
   * <p>Permite sobrescrever os percentuais padrão de mão de obra, custos fixos e margem.
   */
  CustosCalculadosResponse calcularCustos(
   ...
   * de filtro e seletores na tela de receitas.
   */
  List<CategoriaReceitaResponse> listarCategorias();

  /**
   * Busca ingredientes do workspace por nome (autocomplete). Retorna todos os ingredientes quando
   * {@code query} é nulo ou vazio.
   */

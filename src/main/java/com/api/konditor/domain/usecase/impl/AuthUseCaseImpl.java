 ...
 *
 * <p>Concentra <strong>todas</strong> as regras de negócio relacionadas à autenticação: ciclo de
 * vida do refresh token, gerenciamento do cookie HttpOnly, resolução de workspace e emissão de JWT.
 */
@Slf4j
@Service
...
public class AuthUseCaseImpl implements AuthUseCase {
  ...
  public GoogleAuthResponse loginComGoogle(String idToken, HttpServletResponse response) {
    ...
    log.debug(
        "[AUTH] Tokens emitidos para usuário id={} workspace={}",
        usuario.getId(),
        ctx.workspace().getId());
  ...
  public RenovarTokenResponse renovarToken(String refreshToken, HttpServletResponse response) {
    ...
    log.info(
        "[AUTH] Tokens renovados para usuário id={} workspace={}",
        usuario.getId(),
        ctx.workspace().getId());
  ...
  private RefreshTokenJpaEntity validarRefreshToken(String valorToken) {
    ...
    if (token.isRevoked()) {
      log.warn(
          "[SECURITY] Reutilização de refresh token detectada para userId={}. Revogando todas as"
              + " sessões.",
          token.getUserId());
      refreshTokenRepository.revokeAllByUserId(token.getUserId());
      throw new AuthException(
          "Token reutilizado. Todas as sessões foram encerradas por segurança.");
    }
  ...
  /**
   * Cria um novo usuário ou atualiza nome e email caso já exista pelo googleId. A entidade
